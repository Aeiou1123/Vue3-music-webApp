/*
 * 该文件是运行在 Node.js 端的(即后端)，获取数据的基本的思路就是后端代理(因为浏览器有跨域问题，
 * 所以我们可以通过node serve向服务器请求数据，然后再由浏览器向node serve拿到数据)，即提供接口路由供前端页面使用，
    ...
    get(url, {
      ...
      if (data.code === ERR_OK) {
        ...
        for (let i = 0; i < len; i++) {
          ...
          const sliderItem = {}
          // 单个轮播图数据包括 id、pic、link 等字段。从拿到的原始数据里面拿到我们需要的数据
        ...
        // 往前端发送一个标准格式的响应数据，包括成功错误码和数据。(这就是处理好之后返回给前端可以直接用的数据)
          res.json({
            code: ERR_OK,
            result: {
              sliders,
